...
def calcular_h2h(df):
    ...
    for index, row in df.iterrows():
        ...
        previous_encounters = get_previous_encounters(df, winner, loser, row['tourney_date'])
...
def get_previous_matches(df, player_id, date):
    """
...
def get_previous_encounters(df, player1_id, player2_id, date):
    """
...
def main():
    ...
    df_processed = df.pipe(tempo_jogado_dataframe).pipe(calcular_h2h)
